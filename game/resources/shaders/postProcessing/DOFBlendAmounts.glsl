...
void main() {
    ...
    for (int x = 0;x<49;x++){
        float d = texture(u_depthTexture0, v_texcoord + kernel[x] / u_resolution * 8.0).x;
        float depth = (2.0 * u_near * u_far) / (u_far + u_near - d * (u_far - u_near));
        depth -= u_targetDepth;
        depth /= u_depthRange;
        depth = clamp(-depth,0.0,1.0);
        fragColor.r += depth / 49.0;
        // fragColor += texture(u_texture, v_texcoord + kernel[x] / u_resolution * 4.0)/49.0;
    }
    float d = texture(u_depthTexture0, v_texcoord).x;
    float depth = (2.0 * u_near * u_far) / (u_far + u_near - d * (u_far - u_near));
    depth -= u_targetDepth;
    depth /= u_depthRange;
    depth = clamp(depth,0.0,1.0);
