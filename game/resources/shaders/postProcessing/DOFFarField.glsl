...
void main() {
    ...
    for (int x = 0;x<49;x++){
        ...
        float t = texture(u_colorTexture1, v_texcoord + kernel[x] / u_resolution * d).g;
        if (t>0.0){
            fragColor.rgb += texture(u_colorTexture0, v_texcoord + kernel[x] / u_resolution * d).rgb;
            totalWeight += 1.0;
        }
    }
    fragColor.xyz /= totalWeight;
